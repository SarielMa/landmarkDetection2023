#!/usr/bin/env python3
"""
Created on Thu Mar  9 21:36:09 2023
...
#%%
# TODO: Remember to copy Sin function manually to avoid import error
# from ShapeSelfAttention import Sin
class Sin(nn.Module):
...
#%%
'''
def normalize_shape_slow(obj, H, W):
    obj=obj.clone()
    x=obj[:,:,0]
    y=obj[:,:,1]        
    x=(x-511/2)/(511/2)
    y=(y-511/2)/(511/2)
    #x,y in range of -1 to 1
    W=W-1
    H=H-1
    x=0.5*(W+W*x)
    y=0.5*(H+H*y)
    ...
    return obj
#%%
def normalize_shape(obj, H, W):
    #obj: (B,N,2), obj[:,:,0] is x, obj[:,:,1] is y
    #obj is defined in the 512x512 image space
    obj=(obj-511/2)/(511/2) #range: -1 to 1
    #x,y in range of -1 to 1
    W=W-1
    H=H-1
    A=torch.tensor([W-1, H-1], dtype=obj.dtype, device=obj.device)
    A=A.view(1,1,2)
    obj=0.5*(A+A*obj)
    #x range: 0 to W-1, y range 0 to H-1 
    return obj
'''
#%%
...
class TemplateShapeEmbedding(nn.Module):
    ...
    def forward(self, template):
        # template: (B, N, D), template shape, D=pos_dim=2, normalized in range of -1 to 1 in ShapeDeformNet   
